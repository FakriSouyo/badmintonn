...
const Notifikasi = ({ isOpen, onClose, user }) => {
  ...
  const handleNotificationClick = (notif) => {
    if (!isSelectionMode && notif.booking_id) {
      setSelectedBookingId(notif.booking_id);
      setShowBookingHistory(true);
      onClose(); // Tutup modal notifikasi
    }
  };
                    ...
                    {notifications.map((notif) => (
                      <li
                        ...
                        className={`p-3 rounded-md transition-colors ${
                          ...
                          !isSelectionMode && notif.booking_id ? 'cursor-pointer hover:bg-gray-200' : ''
                        }`}
